#!/usr/bin/env python3
"""
Incremental updater for the liminalwriting channel.

1. Parses liminalwriting → liminalwriting_common.md (full refresh)
2. Syncs individual post files in night-posts/:
   - Updates files whose content changed (edited posts)
   - Creates files for brand-new posts
...
COMMON_MD  = Path("/Users/blacktrope/Library/CloudStorage/Dropbox/channels/liminalwriting_common.md")
POSTS_DIR  = Path("/Users/blacktrope/Library/CloudStorage/Dropbox/soshnikov-writing/writing/night-posts")

# ── parser helpers ────────────────────────────────────────────────────────────

def resolve_post_text(msg) -> str:
    """Return post text; media-only messages (photos without caption) yield ""."""
    wp_url = (
...
async def parse_channel(dry_run: bool):
    """Fetch all posts from liminalwriting and write to COMMON_MD."""
    async with TelegramClient(SESSION_NAME, API_ID, API_HASH) as client:
        ...
        if dry_run:
            print(f"(dry-run) Would write {len(posts)} text posts to {COMMON_MD}")
            return len(posts)
        ...
        print(f"Saved {len(posts)} text posts to: {COMMON_MD}")
        return len(posts)
...
def sync_posts(dry_run: bool):
    ...
    for i, post in enumerate(posts, start=1):
        ...
        if out_path.exists():
            ...
            # Content differs → post was edited in the channel
            ...
            # New post — file doesn't exist yet
            preview = post.splitlines()[0][:70]
...
def main():
    parser = argparse.ArgumentParser(
        description="Incremental updater: parse liminalwriting, then knife new posts."
    )
